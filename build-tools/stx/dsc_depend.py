...
-)  Cyclical dependencies are dealt via config file. Any circular dependency not listed
...
def get_aptcache(rootdir):
    ...
    try:
        apt_cache = apt.Cache(rootdir=apt_rootdir)
...
class Cycle_dsc_order():
    ...
    def __init__(self, circular_meta_info, logger):
        ...
        if circular_meta_info[0] != set(circular_meta_info[1]):
            ...
            raise Exception('CYCLE META INFO ERROR')
        self.logger = logger
    ...
    def get_build_able_pkg(self):
        # Get packages can be built. Currently, CYCLE group does not support parallel build.
        if self.building_index >= 0:
            self.logger.info('Previous package still in building stage...')
            return None
        if self.next_index < 0:
            self.logger.debug('Cycle group, no more package need to build')
            return None

        self.building_index = self.next_index
        self.next_index += 1
        if self.next_index == len(self.build_order):
            self.logger.debug('Cycle group will build the last package')
            self.next_index = -1
    ...
    def pkg_accomplish(self, pkg_name):
        ...
        if pkg_name != self.build_order[self.building_index]:
            self.logger.error('Cycle group, %s does not in building stage' % pkg_name)
            return False
        self.building_index = -1
    ...
    def pkg_fail(self, pkg_name):
        ...
        if pkg_name != self.build_order[self.building_index]:
            self.logger.error('Cycle group, %s is not in building stage' % pkg_name)
            return False
        self.building_index = -1
        self.next_index -= 1
        return True
...
class Simple_dsc_order():
    ...
    def __depth_t(self, node, dependencies, chain):
        '''
        Search the dependency tree. Once cyclical dependency detected, dump it.
        '''
        if node in chain:
            ...
            raise Exception('UNEXPECTED CYCLE DEPENDENT.')
        chain.append(node)
    ...
    def __set_priority(self):
        ...
        source package. Once cyclical dependency find, dump all related source
        ...
        #    cycle, there must be circular dependency.
        while tmp_d_on:
            shrink = False
            for key in list(tmp_d_by.keys()):
                ...
                if not tmp_d_by[key]:
                    ...
                    tmp_d_by.pop(key)
                    shrink = True
            # cyclical dependency detected,dump it and raise an exception.
            if not shrink:
                chain = []
...
class Cycle_break():
    ...
    1) separate source packages into different sets:
        ...
        Cycle: packages of a circular dependency, build order defined by config file
    2) Let OBS get correct packages
    '''
    def __init__(self, logger, meta_info, cycle_conf_file=None):
        '''
        package_grp: seperate all packages in groups, define them as dictionaries:
        package_grp: [group_0, group_1, group_2...]
        group_x:{
                'grp_type': 'Simple'/'Cycle'
                'grp_meta_info': A list, base information of the group
                'grp_order': Object of Cycle_dsc_order/Simple_dsc_order
        ...
        self.cycle_conf = []
        if cycle_conf_file:
            self.__get_cycle_conf(cycle_conf_file)
        self.__grouping(meta_info)
        self.current_group_index = 0
        self.package_grp[0]['grp_state']['build_state'] = 'building'

    def __get_cycle_conf(self, cycle_conf_file):
        '''
        Read file "cycle_conf_file" and store cycle info into self.cycle_conf
        self.cycle_conf[n][0]: set. A set of packages construt a circular dependency
        self.cycle_conf[n][1]: list. Build order of those packages above
        '''
        src_set = 'SRC SET:'
        build_order = 'BUILD ORDER:'
        if not os.access(cycle_conf_file, os.R_OK):
            self.logger.error('Cycle conf file %s not read-able.' % cycle_conf_file)
            return
        try:
            f_cycle_conf = open(cycle_conf_file, 'r')
        except Exception as e:
            print(e)
            raise Exception('Cycle config file open failed')
        # scan the cycle config file
        want_set = True
        for line in f_cycle_conf:
            line = line.strip().split('#')[0]
            if line.startswith(src_set):
                if not want_set:
                    self.logger.error('Want key word "BUILD ORDER:": %s' % line)
                    raise Exception('CYCLE CONFIG FILE ERROR')
                ...
                if set(src_list) != srcs or not srcs:
                    ...
                    raise Exception('CYCLE CONFIG FILE ERROR')
                want_set = True
                self.cycle_conf.append([srcs, src_list])
    ...
    def __get_cycle_group(self, pkgs, meta_info):
        '''
        Get a cycle group from a set of source packages. Scan the self.cycle_conf
        get the fist set of packages that not build depend on others in "pkgs"
        Construct this set of packages as a 'Cycle_group' dictionary and append
        it into self.package_grp.
        pkgs: The original group of the source packages
        meta_info: Meta_info of all relate packages. Packages in "pkgs" must
                   exist in "meta_info", but packages in "meta_infp" may not
                   exist in "pkgs"
        Return a set of packages, not been selected into the cycle group.
        ...
        # Scan self.cycle_conf to find a build-able package set in it
        for cycle_pkgs in self.cycle_conf:
            # cycle_pkgs : [set, list]
            ...
            # Later, there is a set in cycle_conf {a,d}, for {a,b,c,d} had already
            # been checked and failed, so wee shouldn't check {a,d} any more.
            superset_checked = False
            for checked in checked_set:
                if checked.issuperset(cycle_pkgs[0]):
                    superset_checked = True
                    break
            if superset_checked:
                checked_set.append(cycle_pkgs[0])
                continue

            # cycle_pkgs[0]: a set of source package name, all of them exist in
            # "pkgs". Check to see if they can build now.
            if cycle_pkgs[0].issubset(set(dep_on.keys())):
                dep_on_set = set()
                for pkg in cycle_pkgs[0]:
                ...
                if dep_on_set == cycle_pkgs[0]:
                    ...
                    for pkg in cycle_pkgs[1]:
                        real_meta_info[1].append(dict_pkg_meta[pkg])
                    group_order = Cycle_dsc_order(real_meta_info, self.logger)
                    group_dict = dict()
                    group_dict['grp_type'] = 'Cycle'
                    group_dict['grp_meta_info'] = real_meta_info.copy()
                    group_dict['grp_order'] = group_order
                    group_dict['grp_state'] = dict()
                    group_dict['grp_state']['build_state'] = 'crude'
                    group_dict['grp_state']['num_pkg'] = len(cycle_pkgs[0])
                    group_dict['grp_state']['num_build'] = len(cycle_pkgs[1])
                    group_dict['grp_state']['num_accomplish'] = 0
                    self.package_grp.append(group_dict)
                    return ret_pkgs
        return ret_pkgs

    def __depth_t(self, node, dependencies, cycle_chain):
        # Search the dependency tree. Once a cyclical dependency detected, raise exception.
        if node in cycle_chain:
            raise Exception('CYCLICAL DEPENDENCY DETECTED.')
        cycle_chain.append(node)
        if node in set(dependencies.keys()) and dependencies[node]:
            for nd in dependencies[node]:
                self.__depth_t(nd, dependencies, cycle_chain)
                cycle_chain.pop()
    ...
    def __get_one_cycle(self, depends):
        # Try to find a cyclical dependency
        find_cycle = False
        cycle_chain = list()
        ...
        try:
            for node in list(depend_on.keys()):
                cycle_chain.clear()
                self.__depth_t(node, depend_on, cycle_chain)
        except Exception as e:
            # Find a single cycle
            self.logger.debug('%s' % e)
            find_cycle = True
        if not find_cycle:
            # self.logger.debug('No cyclical dependency found')
            return set()
        # Find all packages belong to this cycle.
        # For any package, in case both its depend_on and depend_by packages
        # contains any package of this cycle, this package is also part of
        # this cycle.
        new_pkgs_in_cycle = set()
        for pkg in depend_on.keys():
            if pkg not in cycle_chain:
                deps_on = self.__get_all_deps(pkg, depend_on)
                deps_by = self.__get_all_deps(pkg, depend_by)
                dep_on_and_cycle = deps_on & set(cycle_chain)
                dep_by_and_cycle = deps_by & set(cycle_chain)
                if dep_on_and_cycle and dep_by_and_cycle:
                    new_pkgs_in_cycle.add(pkg)
        return new_pkgs_in_cycle.union(set(cycle_chain))
    ...
    def __dump_circular_dep(self, pkgs, meta_info):
        ...
        # Find and dump all cyclical dependency
        while True:
            # Get one cyclical dependency
            pkgs = self.__get_one_cycle(depend_on)
            if not pkgs:
                break
            self.logger.error('Cyclical dependency: %s' % pkgs)
    ...
    def __grouping(self, meta_info):
        ...
        while(len(pkgs) != 0):
            ...
            if not pkgs:
                return
            pkgs = self.__get_cycle_group(pkgs, meta_info)
    ...
    def get_state(self):
        ...
        build_state['cycle_group_num'] = l_grp
        return build_state


class Dsc_build_order(Cycle_break):
    '''
    Manage the build order of a set of dsc files.
    '''

    def __init__(self, dsc_list, logger, cycle_conf_file=DEFAULT_CYCLE_CONFIG):
        '''
        ...
        super().__init__(logger, self.meta_info, cycle_conf_file)
...
class Pkg_build(Cycle_break):
    '''
    Choose packages need to be built and manage the build order of them.
    '''
    def __init__(self, meta_info, target_pkgs, logger, cycle_conf_file=DEFAULT_CYCLE_CONFIG):
        '''
        meta_info = [dict, dict]
        meta_info[0] defines binary packages can be built from a source package
        meta_info[1] defines binary packages that depend on by a source package
    ...
    def __get_meta_info(self, meta_info, target_pkgs):
        '''
        Construct meta_info for Cycle_break
        '''
